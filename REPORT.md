...
- throughput_rps: показывает, какую реальную нагрузку выдерживает сервис
- latency_p50_ms: обычная, типичная задержка для пользователя.
- latency_p95_ms latency_p99_ms: показывают редкие медленные ответы, которые важны в проде
- cpu_percent_mean memory_mb_mean: помогают понять, какой вариант будет дешевле по ресурсам.
...
всё работает стабильно, но скорость средняя.
